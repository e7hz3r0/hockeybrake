...
describe 'HockeyLog Exception Tracing' do
  it 'trace standard exception' do
    begin
      ...
      notice = Airbrake.send(:build_notice, $!)
  ...
  it 'raise specific exception when trace failed' do
    expect {
      ...
      expect(error.message).to start_with("An exception was thrown during handling of the exception from the HockeyBrake injector")
    end
  end
end
